...
filename = 'support_vector_machine_big_data.sav'
pickle.dump(classifier, open(filename, 'wb'))

# test_df = pd.read_csv(train_path, index_col=0).iloc[:,:]
preds = classifier.predict(test_df)

pred_df = pd.DataFrame(preds, columns=['label'])

pred_df.to_csv("support_vector_machine_big_data.csv", index=True, index_label='Id')
